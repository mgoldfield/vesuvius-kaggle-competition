#!/bin/bash
# Overnight gpu0: margin distance loss with original labels
# Frozen encoder, margin=3, power=2, w_dist=0.02
set -uo pipefail
...
RUN_NAME="frozen_margin_dist"

echo "" >> "$LOG"
echo "=== Margin Distance Training ===" | tee -a "$LOG"
...
if [[ $TRAIN_EXIT -ne 0 ]]; then
    echo "!!! Training failed (exit=$TRAIN_EXIT) — continuing to eval !!!" | tee -a "$LOG"
fi
echo "--- Training COMPLETE at $(date) ---" | tee -a "$LOG"

# ── Evaluate checkpoints ──
echo "" | tee -a "$LOG"
echo "=== Evaluating Checkpoints ===" | tee -a "$LOG"
