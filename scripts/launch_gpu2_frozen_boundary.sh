#!/bin/bash
# GPU2 experiment: Frozen encoder + dist_sq loss + boundary loss
...
#
# Run on gpu2:
#   nohup bash scripts/launch_gpu2_frozen_boundary.sh > logs/train_eval_frozen_boundary.log 2>&1 &
#
set -euo pipefail
cd /workspace/vesuvius-kaggle-competition

VENV=/workspace/venv/bin/python3
RUN_NAME="frozen_boundary"
LOG="logs/train_eval_${RUN_NAME}.log"
mkdir -p logs checkpoints

echo "=== Frozen Encoder + Boundary Loss Training ===" | tee "$LOG"
