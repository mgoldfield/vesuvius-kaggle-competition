#!/bin/bash
# GPU1 experiment: Discriminative LR + dist_sq loss + boundary loss
# Per-layer LRs: enc=lr/100, vit=lr/10, dec=lr/10, head=lr
# dist_sq loss: skel=0.75, fp=1.5, dist=2.0, power=2.0
# boundary loss: weight=0.3 (matching SegResNet v9)
#
# Run on gpu1:
#   tmux new-session -d -s train 'cd /workspace/vesuvius-kaggle-competition && bash scripts/launch_gpu1_discrim_boundary.sh > logs/train_eval_discrim_boundary.log 2>&1'
#
set -euo pipefail
cd /workspace/vesuvius-kaggle-competition

VENV=/workspace/venv/bin/python3
RUN_NAME="discrim_boundary"
LOG="logs/train_eval_${RUN_NAME}.log"
mkdir -p logs checkpoints

echo "=== Discriminative LR + Boundary Loss Training ===" | tee "$LOG"
