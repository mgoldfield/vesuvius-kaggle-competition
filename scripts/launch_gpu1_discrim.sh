#!/bin/bash
# GPU1 restart experiment: Discriminative LR + dist_sq loss
# Encoder=5e-7, ViT=5e-6, Decoder=5e-6, Head=5e-5
# dist_sq loss: skel=0.75, fp=1.5, dist=2.0, power=2.0
#
# Run on gpu1 after restart with more storage:
#   nohup bash scripts/launch_gpu1_discrim.sh > logs/train_eval_discrim_dist_sq.log 2>&1 &
#
set -euo pipefail
cd /workspace/vesuvius-kaggle-competition

VENV=/workspace/venv/bin/python3
RUN_NAME="discrim_dist_sq"
LOG="logs/train_eval_${RUN_NAME}.log"
mkdir -p logs checkpoints

echo "=== Discriminative LR Training ===" | tee "$LOG"
