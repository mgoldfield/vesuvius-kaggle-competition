...
Run 11: 3-fold ensemble (SegResNetDSAttn) + TTA + hysteresis + 160^3.
Models exported via torch.jit.trace (no MONAI dependency needed).
Probability averaging across 3 fold models before thresholding.
"""
import torch
import numpy as np
...
# ── Load Ensemble Models (traced — no MONAI needed) ───────
models = []
for i in range(N_FOLDS):
    weights_path = WEIGHTS_DIR / f"best_segresnet_v11_fold{i}_traced.pt"
...
# ── Ensemble Inference ────────────────────────────────────
def ensemble_inference(models, volume, patch_size=160, stride=80, device="cuda", use_tta=True):
    """Run all models on the volume, average probabilities."""
    infer_fn = sliding_window_inference_tta if use_tta else sliding_window_inference
    probs = []
    for model in models:
        prob = infer_fn(model, volume, patch_size, stride, device)
        probs.append(prob)
    return np.mean(probs, axis=0)
...
print(f"Inference: {len(models)} models x {'7 TTA' if USE_TTA else '1'} = "
      f"{len(models) * (7 if USE_TTA else 1)} forward passes per volume")
...
for i, row in test_df.iterrows():
    ...
    prob = ensemble_inference(models, img, patch_size=PATCH_SIZE, stride=STRIDE,
